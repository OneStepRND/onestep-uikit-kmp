...
// ── Tag catalog snapshot (OS-17546) ─────────────────────────────────────────────
// fixtures/tag_catalog_sample.json is the backend tag catalog for every activity × en_US/he_IL,
// generated with the backend's own resolver (shared with the Android SDK's testing app). The
// tests read it as a Kotlin constant rather than a file so they run unchanged on the JVM host and
// the iOS simulator, neither of which gives commonTest a portable resource path.
val generateTagCatalogSample by tasks.registering {
    ...
    doLast {
        ...
        val chunks = text.chunked(8_000).joinToString(",\n") { chunk ->
            "    \"" + chunk
                .replace("\\", "\\\\")
                .replace("\"", "\\\"")
                .replace("\$", "\\\$")
